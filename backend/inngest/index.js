...
import sendEmail from "../confiq/nodemailer.js";

// Create a client to send and receive events
export const inngest = new Inngest({ id: "ems-application" });

// Auto check-out employees
const autoCheckOut = inngest.createFunction(
  { id: "autoCheckOut", triggers: [{event: "employees/check-out"}]},
  async ({ event, step }) => {
...
// Send Email to admin , if admin doesn't take action on leave within 24 hours
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step })=> {
    ...
    if(leaveApplication?.status === "PENDING") {
      ...
      // send reminder emai' to admin to take action on leave application
      await sendEmail({
...
// Cron: Check attendance at 11:30 AM IST (06:00) UST and email ABSENT employee

const attendanceReminderCron = inngest.createFunction(
  { id: "absent-Reminder-Cron",triggers:[{cron: "TZ=Asia/Kolkata 30 11 * * *"}]}, // 06:00 UTC = 11:30 IST
  async({ step }) => {
    ...
    // Filter absent employees (not on leave & not check in)
