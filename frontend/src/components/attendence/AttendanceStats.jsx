...
const AttendanceStats = ({history}) => {
  ...
  const totalLate = history.filter((h)=>h.status === "PRESENT" || h.status === "LATE").length;
  ...
  return (
    <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
      {stats.map((s)=>(
        <div key={s.label} className='card card-hover p-5 sm:p-6 flex items-center relative gap-4 overflow-hidden group'>
